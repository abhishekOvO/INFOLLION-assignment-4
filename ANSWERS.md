...
`Express Surcharge = $5.00 + ($0.10 × distance_km)`

*(If a `SAVE10` coupon is present, a 10% discount is applied to the grand total, scaling this surcharge to `0.90 × ($5.00 + $0.10 × distance_km)`).*
...
7. **Reverse-Engineering the Formula:** Performed linear regression on `v1_total` and `v2_total` across categories. Determined that `v1` express fee was \$5.00 + \$0.10/km, and proved that the exact `v2_total - v1_total` delta for fragile express orders matched `$5.00 + ($0.10 × distance_km)` (with maximum residual error $\le$ \$0.026).
